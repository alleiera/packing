...
class PDFExporter(FPDF):
    def __init__(self, *args, **kwargs):
        ...
        # Register unicode fonts to support Turkish characters
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        self.add_font("NotoSans", style="", fname=os.path.join(base_dir, "fonts/NotoSans-Regular.ttf"))
        self.add_font("NotoSans", style="B", fname=os.path.join(base_dir, "fonts/NotoSans-Bold.ttf"))
    ...
    def header_section(self, info):
        ...
        self.cell(90, 5, info.get('doc_no', ''), 1, 1)
    ...
    def sections(self, info):
        ...
        self.set_font('NotoSans', '', 8)
        self.cell(30, 4.5, 'Company:', 1, 0); self.cell(160, 4.5, info.get('con_company', ''), 1, 1)
        self.cell(30, 4.5, 'Address:', 1, 0); self.cell(160, 4.5, info.get('con_address', ''), 1, 1)
        self.cell(30, 4.5, 'Tel:', 1, 0); self.cell(160, 4.5, info.get('con_tel', ''), 1, 1)
        self.ln(1)
        self.set_font('NotoSans', 'B', 9)
        self.cell(0, 5, 'SHIPPER / EXPORTER', 1, 1, 'L', True)
        self.set_font('NotoSans', '', 8)
        self.cell(30, 4.5, 'Company:', 1, 0); self.cell(160, 4.5, info.get('ship_company', ''), 1, 1)
        self.cell(30, 4.5, 'Address:', 1, 0); self.cell(160, 4.5, info.get('ship_address', ''), 1, 1)
        self.cell(30, 4.5, 'Tel:', 1, 0); self.cell(160, 4.5, info.get('ship_tel', ''), 1, 1)
        self.ln(1)
        self.set_font('NotoSans', 'B', 9)
        ...
        for label, key in fields:
            ...
            self.cell(150, 4.5, info.get(key, ''), 1, 1)
        self.ln(3)
    ...
    def draw_table(self, headers, data):
        ...
        for i, h in enumerate(headers): self.cell(ws[i], 6, h, 1, 0, 'C', True)
        self.ln()
        self.set_font('NotoSans', '', 8)
        for r in data:
            for i, v in enumerate(r): self.cell(ws[i], 5, str(v) if v is not None else "", 1, 0, 'C')
            self.ln()
    ...
    def draw_page_footer(self, info):
        ...
        self.set_font('NotoSans', 'B', 8)
        self.cell(0, 4, info.get('ship_company', ''), 0, 1, 'C')
        self.set_font('NotoSans', '', 7)
        addr = info.get('ship_address', '')
